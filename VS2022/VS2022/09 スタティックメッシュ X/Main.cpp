...
HRESULT InitDirect3D()
{
	...
	//メッシュ作成
	g_pMesh=new CD3DXMESH;
	g_pMesh->Init(g_hWnd,g_pDevice,g_pDeviceContext,"Chips.x");
...
void App()
{
	...
	// ビュートランスフォーム
	D3DXVECTOR3 vEyePt( 0.0f, 0.0f,-0.5f ); //視点位置
	...
	//レンダリング
	g_pMesh->Render(View,Proj,D3DXVECTOR3(1,1,1),vEyePt);
	g_pMesh->m_fYaw+=0.001;
