		...
		{			
			...
			if (pMaterials[i].pTextureFilename != NULL)
			{
				...
				strcpy_s(pMeshContainer->pMaterials[i].pTextureFilename,
					512,pMaterials[i].pTextureFilename);
			}
		}
	}
	else
...
//
//初期化
HRESULT CD3DXSKINMESH::Init(CD3DXSKINMESH_INIT* si)
{
	m_hWnd=si->hWnd;
	...
	//ブロブからバーテックスシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"SKIN_MESH.hlsl",NULL,NULL,"VSSkin","vs_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
	...
	//ブロブからピクセルシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"SKIN_MESH.hlsl",NULL,NULL,"PSSkin","ps_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
		MessageBox(0,L"hlsl読み込み失敗",NULL,MB_OK);
		return E_FAIL;
	}
...
HRESULT CD3DXSKINMESH::CreateFromX(CHAR* szFileName)
{
	...
	for(int i=0;i<m_dwNumMaterial;i++)
	{
		...
		if(name)
		{
			strcpy_s(m_pMaterial[i].szTextureName,sizeof(m_pMaterial[i].szTextureName),name);
		}
		//テクスチャーを作成
...
void CD3DXSKINMESH::Render()
{
	...
	{
		m_pD3dxMesh->m_pAnimController->AdvanceTime(0.0006,NULL);
	}
	D3DXMATRIX m;
