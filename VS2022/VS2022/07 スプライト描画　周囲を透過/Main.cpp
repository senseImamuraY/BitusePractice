...
//定数定義
#define NAME L"スプライト描画 不要な色を消して絵だけを描画"
...
HRESULT InitDirect3D()
{
	...
	//バーテックスシェーダー作成
	MakeShader("Shader.hlsl","VS","vs_5_0",(void**)&g_pVertexShader,&pCompiledShader);;	
	//頂点インプットレイアウトを定義	
	...
	//ピクセルシェーダー作成
	MakeShader("Shader.hlsl","PS","ps_5_0",(void**)&g_pPixelShader,&pCompiledShader);
	...
	//テクスチャー作成
	D3DX11CreateShaderResourceViewFromFile(g_pDevice, L"audi.tif", NULL, NULL, &g_pTexture, NULL);//pngでもいい
