...
HRESULT InitDirect3D()
{
	...
	//メッシュ作成
	g_pMesh=new CD3DXMESH_ANIM;
	g_pMesh->Init(g_hWnd,g_pDevice,g_pDeviceContext,"Robot.x");
...
void App()
{
	...
	// ビュートランスフォーム
	D3DXVECTOR3 vEyePt( 0.0f, 1.0f,-3.0f ); //視点位置
	...
	//レンダリング
	g_pMesh->Render(View,Proj,D3DXVECTOR3(1,1,-1),D3DXVECTOR3(0,0,-1));
	g_pMesh->m_fYaw+=0.0002;
	g_pMesh->m_pAnimController->AdvanceTime(0.00015,NULL);
