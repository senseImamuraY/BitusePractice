	...
	if (NumMaterials > 0)
	{
		...
		for (iMaterial = 0; iMaterial < NumMaterials; iMaterial++)
		{
			if (pMaterials[iMaterial].pTextureFilename != NULL)
			{
				...
				strcpy_s(pMeshContainer->pMaterials[iMaterial].pTextureFilename,
					sizeof(pMeshContainer->pMaterials[iMaterial].pTextureFilename),pMaterials[iMaterial].pTextureFilename);
			}
		}
	}
	else
	...
	for( DWORD i=0; i<pAppMesh->dwNumMaterial; i++ )
	{
			...
			lstrlenA(pFrame->pMeshContainer->pMaterials[i].pTextureFilename) > 0 )
		{
			...
			strcpy_s(pAppMesh->pMaterial[i].szTextureName,
				sizeof(pAppMesh->pMaterial[i].szTextureName),pFrame->pMeshContainer->pMaterials[i].pTextureFilename);
			//テクスチャーを作成
